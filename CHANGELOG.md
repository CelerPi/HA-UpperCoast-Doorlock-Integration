...
- **优化手机端呼叫接听界面**
  - 手机宽度下呼叫弹窗改为更适合 HA App 的接听页布局
  - 呼入时显示“接听 / 解锁 / 挂断”三按钮，通话中显示“解锁 / 挂断”
  - 视频窗口固定为 4:3，匹配门口机 640x480 实时画面
  - 静态预览页新增手机尺寸弹窗预览和 640x480 测试画面
...
- **支持更像门铃的通知声音设置**
  - 蓝图新增 iOS 通知声音文件名，可使用 `default` 或 HA App 已导入的声音完整文件名
  - 蓝图新增 Android 通知频道，可在 Android 系统/HA App 通知频道中配置长铃声
  - 高级通知继续支持 Critical Alert 音量 1.0

## v0.4.3

- **恢复手机 App 高级通知按钮模式**
  - 蓝图新增可选“手机 App 高级通知服务”，支持填 `notify.mobile_app_*`
  - 填入高级通知服务后，来电通知会带接听/查看、解锁、挂断按钮、跳转和摄像头预览
  - 保留 `notify.*` 通知实体作为基础通知兜底

## v0.4.2

- **修复 `notify.send_message` 不接受扩展 data 导致自动化失败**
  - 新版 HA 通知实体只保留基础通知字段，避免 `extra keys not allowed @ data['data']`
  - 手机通知先保证可靠送达；接听、解锁、挂断回到门禁卡片内操作
...
- **修复 WebSocket 前端断开时污染 HA 日志**
  - 前端页面关闭、刷新或移动端切后台时，WebSocket 代理不再向已关闭连接写入数据
  - 将前端和 addon 任一侧正常断开视为会话结束，避免 `Cannot write to closing transport`
  - 版本号按 `0.3.9 -> 0.4.0` 递增

## v0.3.9

- **修复手机来电通知不触发**
  - 蓝图现在同时监听 `binary_sensor.vds_call_status` 状态变化和自定义事件
  - 只要呼叫状态实体从 `off` 变为 `on`，就会推送手机通知
  - 增加“门禁呼叫状态实体”输入项，兼容用户自定义实体 ID

## v0.3.8

- **修复 HACS 安装后找不到手机通知蓝图**
  - 将手机来电通知蓝图打包进集成目录
  - 集成启动时自动安装/更新到 HA 的 `blueprints/automation/uppercoast_doorlock/`
  - 更新后重启 Home Assistant 即可在蓝图列表中看到「云海湾门禁来电通知」
